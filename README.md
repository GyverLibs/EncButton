...
- [Документация](#docs)
  ...
  - [Событийное программирование](#callback)
...
// прочитать текущее значение кнопки (без дебаунса) с учётом setButtonLevel
bool read();
...
// прочитать значение кнопки с учётом setButtonLevel
bool readBtn();
...
> Опрос кнопки выполняется с учётом настроенного ранее уровня кнопки (setButtonLevel)! Вручную дополнительно инвертировать логику не нужно

```cpp
void setup() {
  // btn.setButtonLevel(LOW); // можно настроить уровень

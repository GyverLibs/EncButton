...
- **NEW!** Для работы только с кнопкой, а также удобного наследования алгоритмов кнопки в другие классы и библиотеки рекомендую использовать библиотеку [VirtualButton](https://github.com/GyverLibs/VirtualButton). Она содержит те же алгоритмы опроса, что и здесь, но энкодер полностью убран и код ещё больше облегчён и оптимизирован. Методы опроса кнопки полностью совместимы с EncButton.
...
> Примечание: при использовании китайских модулей включайте режим пинов INPUT. На некоторых МК (например lgt328) внутренняя подтяжка создаёт делитель напряжения с внешней подтяжкой модуля и сигнал GND не доходит до пина!

...
# EncButton
Ультра лёгкая и быстрая библиотека для энкодера, энкодера с кнопкой или просто кнопки с Arduino
- Максимально быстрое чтение пинов для AVR (ATmega328/ATmega168, ATtiny85/ATtiny13)
- Лёгкий вес во Flash и SRAM памяти
- Оптимизированные алгоритмы опроса кнопки и энкодера
...
- Ручной опрос действий или режим обработчиков
...
// A, B, KEY: номера пинов, для изменения направления энкодера поменяй местами A и B
...
void attachClicks(uint8_t amount, void (*handler)());   // подключить обработчик на количество кликов (может быть только один), до 16 кликов!
...
### Опрос пинов
Опрос пинов энкодера/кнопки, расчёт таймаутов и вызов коллбэков происходит внутри функции `tick()`. Эту функцию нужно **однократно** вызывать в основном цикле программы `loop()`.
```cpp
void loop() {
  btn.tick();
}
